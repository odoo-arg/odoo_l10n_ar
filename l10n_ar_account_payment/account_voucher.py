...
class account_voucher(osv.osv):
    ...
    def _get_payment_lines_amount(self, payment_line_ids):
    ...
    def proforma_voucher(self, cr, uid, ids, context=None):
        ...
        # Chequeamos si la writeoff_amount no es negativa y que el amount del voucher sea mayor que 0
        writeoff_amount = self.read(cr, uid, ids, ['writeoff_amount'], context=context)[0]['writeoff_amount']
        voucher_amount = self.read(cr,uid, ids, ['amount'], context=context)[0]['amount']
        ...
        if round(voucher_amount, 2) <= 0.0:
            raise osv.except_osv(_("Validate Error!"), _("Cannot validate a voucher with an amount of 0 or less!"))
